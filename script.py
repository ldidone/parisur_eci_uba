                    ...
                    for total in self.total_strings:
                         if total.upper() in res[1].upper():
                              try:
                                   ...
                                   print(f'Error ocurred with the total of invoice_number: {invoice_number}')
                                   self.error_totals.append((invoice_number, res[1]))
                                   continue
          
          if invoice_number:
               self.df.loc[self.df.invoice_number == invoice_number, 'total_charged'] = total_charged
     
     def export_results(self):
          print('Saving results...')
          self.df.rename(columns={'invoice_number': 'Invoice Number',
                                   'total_charged': 'Total Charged'}, inplace=True)
          full_output_path = f'{self.base_output_path}invoice_numbers_{round(time.time())}.csv'
